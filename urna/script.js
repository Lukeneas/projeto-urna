...
let numero = "";
//servirá no IF quando CONFIRMAR
...
function comecarEtapa() {
  ...
  //zera numero no html e memoria, quando clicar em CORRIGE
  let numeroHtml = "";
  numero = "";
  votoBranco = false;
...
//so pode votar em branco se nao tiver clicado em nada
function branco() {
  //limitando
  if (numero === "") {
    votoBranco = true;
...
function confirma() {
  ...
  if (votoBranco === true) {
    votoConfirmado = true;
    //preenche ARRAY VOTOS
    votos.push({
      etapa: etapas[etapaAtual].titulo,
      voto: "branco",
    });
  }
  //CASO NUMEROS PREENCHIDOS
  ...
  if (votoConfirmado) {
    //pular etapa
    etapaAtual++;
    //se etapa atual existe
    if (etapas[etapaAtual] !== undefined) {
      comecarEtapa();
    } 
    //se nao existe finaliza
    else {
